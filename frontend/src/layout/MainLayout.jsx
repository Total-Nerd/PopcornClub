...
const MainLayout = () => {
  ...
  useEffect(() => {
    // Automatically trigger calendar prefetching for past month to next 6 months on mount
      ...
      try {
        ...
        if (result.success) {
          ...
          if (result.events && result.events.length > 0) {
            useCalendarStore.getState().mergeEvents(result.events);
          }
        }
      } catch (err) {
    ...
    // Quick non-blocking delay to let initial route render smoothly
