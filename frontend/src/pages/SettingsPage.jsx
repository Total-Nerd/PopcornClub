...
import { 
  ...
  Palette, User, UserPlus, LogOut, Copy, ShieldAlert, Key, Lock, Mail, Sparkles, Sliders, EyeOff
} from 'lucide-react';
...
const SettingsPage = () => {
  ...
  const handleUnhideItem = async (item) => {
    try {
      ...
      console.error(err);
      setError('Failed to unhide item');
          ...
          {activeTab === 'hidden' && (
            <div className="settings-card">
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '24px' }}>
                <div style={{ padding: '8px', background: 'var(--overlay)', borderRadius: '8px' }}>
                  <EyeOff size={24} style={{ color: 'var(--text-color)' }} />
                </div>
                <div>
                  <h3 style={{ margin: 0, fontSize: '1.25rem' }}>Hidden Items</h3>
                  <p style={{ margin: '4px 0 0', color: 'var(--text-muted)', fontSize: '0.9rem' }}>
                    Movies and shows you have chosen to hide from your calendar or library views.
                  </p>
                </div>
              </div>

              {hiddenItems.length === 0 ? (
                <div style={{ padding: '32px', textAlign: 'center', background: 'var(--overlay)', borderRadius: '12px' }}>
